...
public class MySQLProvider extends ProviderBase{
    ...
    */
    ...
    @Override
    public int AddUser(String userName, String email, String password, int isLocked, String createDate, String createBy, int roleID) {
        try {
            Connection conn = DBConnection.getConnection();
            PreparedStatement pst = null;
           // the mysql insert statement
            String query = "INSERT INTO tbl_user (userName, email, password, isLocked, createDate, createBy, roleID)"+
                            " values (?, ?, ?, ?, ?, ?, ?)";
            // create the mysql insert preparedstatement
            pst = conn.prepareStatement(query);
    ...
    @Override
    public int EditUser(String userName, String email, int isLocked, String createDate, String createBy, int roleID) {
        try {
            Connection conn = DBConnection.getConnection();
            String query = "UPDATE tbl_user SET email=?,isLocked=?, createDate=?, createBy=?, roleID=?"+
                            "WHERE userName=?";
            PreparedStatement pst = conn.prepareStatement(query);
            pst.setString(1, email);
            pst.setInt(2, isLocked);
            pst.setString(3, createDate);
            pst.setString(4, createBy);
            pst.setInt(5, roleID);
            ...
            System.out.println("Edit user error: "+ e.getLocalizedMessage());
        }
        return 0;
    }
    ...
    public int RemoveUser(String userName) {
        try {
            ...
            System.out.println("remove user error: "+ e.getLocalizedMessage());
        }
        return 0;
    }
    ...
    public User getUser(String userName) {
        try {
            ...
            ResultSet rs = null;
            PreparedStatement pst = null;
            String sql="SELECT u.userName, u.email, u.password, u.isLocked, u.createDate, u.createBy, u.roleID, r.roleName, "
                + "ui.firstName, ui.lastName, ui.telephone, ui.profile "
                + "FROM tbl_user u "
                + "JOIN tbl_user_info ui "
                + "ON u.userName = ui.userName "
                + "JOIN tbl_role r "
    ...
    public ArrayList<User> getAllUsers() {
       try {
            ...
            ResultSet rs = null;
            PreparedStatement pst = null;
            String sql="SELECT u.userName,u.email,u.password,u.isLocked,u.createDate,u.createBy,u.roleID," +
                        "ui.firstName,ui.lastName,ui.telephone,ui.profile,r.roleName " +
                        "FROM tbl_user u " +
                        "JOIN tbl_user_info ui " +
                        "ON u.userName = ui.userName " +
                        "JOIN tbl_role r " +
